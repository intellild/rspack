...
use rspack_core::{
  ...
  Compilation, CompilationRenderManifest, CompilerOptions, ConcatenationScope,
  ConcatenationScopeInfoMode, DependencyType, Filename, GenerateContext, GeneratedSource,
  ...
  rspack_sources::{
    BoxSource, PlaceholderKey, PlaceholderSource, RawStringSource, ReplaceSource, RopeSource,
    SourceExt,
  },
};
...
fn render_concatenated_asset_source(
  ...
  let source = RopeSource::from_boxed(vec![
    RawStringSource::from(prefix.clone()).boxed(),
    RawStringSource::from(rendered_namespace_export.to_string()).boxed(),
    RawStringSource::from_static(infix).boxed(),
    exported_content,
    RawStringSource::from_static(suffix).boxed(),
  ])
  .boxed();
